...
<div id="listValues">
    ...
    <input placeholder="Мат.рукоят" list="me" id="mechtype" on:blur={checkList}/>
	<datalist id="me">
		{#each Object.entries(l.mechtype) as i}<option value={i[0]}>
		{/each}
	</datalist>
	<input placeholder="Вид инструм" list="ty" id="type" on:blur={checkList}/>
	<datalist id="ty">
		{#each Object.entries(l.type) as i}<option value={i[0]}>
		{/each}
	</datalist>
	<input placeholder="Мат.бойка" list="ma" id="material" on:blur={checkList}/>
	<datalist id="ma">
	...
	<input id="width" placeholder="Ширина лезвия(мм)" on:input={checkInt} type="number"/>
	<input id="length" placeholder="Длина рукоятки(мм)" on:input={checkInt} type="number"/>
	<input id="weight" placeholder="Вес(гр)" on:input={checkInt} type="number"/>
